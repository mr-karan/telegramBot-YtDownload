...
return json.loads(requests.get(API_BASE + BOT_KEY + '/getUpdates', params = {
    'offset': last_updated + 1
}).text)


def my_hook(d): #if d['status'] == 'downloading': #sendMessage(chat_sender_id, "Wait for a few moments, please")# last_updated = req['update_id']
